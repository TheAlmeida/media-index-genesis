...
const AgendaSlide: React.FC<AgendaSlideProps> = ({ isActive = true, className }) => {
  ...
  return (
    <div className={cn(
      "h-[100dvh] w-[100dvw] bg-gradient-to-br from-gray-50 to-gray-100 relative overflow-hidden flex flex-col pb-[9vh]",
      className
    )}>
      {/* Header - stays at top */}
        ...
        <div className="grid grid-cols-3 gap-8 max-w-7xl w-full">
          {agendaItems.map((item, index) => {
            ...
            return (
              ...
              >
                <div className="bg-white rounded-xl p-10 shadow-lg border border-gray-200 h-full flex flex-col items-center justify-center text-center hover:shadow-xl transition-shadow duration-300">
                  {/* Step Number */}
                  <div className={cn(
                    "w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-2xl mb-8",
                    item.bgColor
                  )}>
                    {index + 1}
                  </div>
